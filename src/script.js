...
btn1.addEventListener("click", function () {
  const table = [1, 5, -5, 26, -265, 874, 48, 523, 52, -334, 31];
  // console.log('Valeur Max dans le tableau : ' + Math.max(...table));
  var n = 0;
  for (i = 0; i < table.length; i++) {
    if (table[i] > table[n]) n = i;
  }
  alert(
    "Valeur Max dans le tableau : " + table[n] + "(details dans la console)"
  );
...
btn2.addEventListener("click", function () {
  ...
  alert(
    "Un des arguments ou la somme des 2 arguments = 1 : " +
      result +
...
btn3.addEventListener("click", function () {
  ...
  alert(
    "Le plus petit interval du tableau est :" + smallInterval,
    element + "(details dans la console)"
  );
  console.log(
    "Le plus petit interval du tableau est :" + smallInterval,
    element,
